        ...
        if (uri === "" || uri === "/") {
          ...
          if (req.method === "POST") {
            ...
            const dbData = JSON.parse(
              fs.readFileSync(DB_FILE_USERS, (err, data) => data)
            );
            fs.writeFileSync(
              DB_FILE_USERS,
              JSON.stringify([...dbData, JSON.parse(createdItem)])
            );
            return createdItem;
          }
        }
      ...
      const body = await (async () => {
        if (uri === "" || uri === "/") {
          ...
          if (req.method === "POST") {
            const createdItem = await drainJson(req);
            const dbData = JSON.parse(
              fs.readFileSync(DB_FILE, (err, data) => data)
            );
            if (
              JSON.parse(createdItem).name === "DELETE_USER_METHOD_RPOST_METHOD"
            ) {
              let index = 0;
              ...
              const deleteArray = fs.writeFileSync(
                DB_FILE,
          ...
          if (req.method === "PUT") {
            ...
            const putArray = fs.writeFileSync(
              DB_FILE,
              JSON.stringify([...dbData])
            );
            return putArray;
