...
export class RegisterComponent implements OnInit {
  constructor(private data: DataService, private lastItem: LastItemService, private router: Router) {}
  coincidence = false
  ...
  ngOnInit(): void {
    ...
    if(localStorage.getItem('userCrmSystemAngular') === 'userCrmSystemAngular') {
  ...
  submit() {
    ...
    if(this.form.value.password === this.form.value.repassword && this.form.value.password !== '' && this.form.controls.mail) {
      this.coincidence = false
      // Если все успешно перенаправляем пользователя к таблице
      this.router.navigate(['/entry'])
      // Отдаем на сервер созданного юзера
      this.data.createUsers({
